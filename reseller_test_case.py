...
keystone_url = "http://10.4.0.56:35357/v3"
project_url = keystone_url + "/projects"
default_token_json = '{ "auth": { "identity": { "methods": [ "password" ], "password": { "user": { "domain": { "name": "Default" }, "name": "admin", "password": "nomoresecrete" } } }, "scope": { "project": { "domain": { "name": "Default" }, "name": "demo" } } } }'
...
admin_role = '30d17462482d4abb852b18122eb76a18'
member_role = '8b87e4741a804c8d8a33e5c0af04c041'
...
def create_hierarchy(token):
    #Create Project A
    a_project_json = project_json('Project_A', 'true', None)
    a_project_id = create_project(a_project_json, token)
    
    #Create Project B
    b_project_json = project_json('Project_B', 'true', a_project_id)
    b_project_id = create_project(b_project_json, token)
    
    #Create Project C
    c_project_json = project_json('Project_C', 'false', a_project_id)
    c_project_id = create_project(c_project_json, token)
    
    #Create Project D
    d_project_json = project_json('Project_D', 'false', b_project_id)
    d_project_id = create_project(d_project_json, token)
    
    #Create Project E
    e_project_json = project_json('Project_E', 'false', b_project_id)
    e_project_id = create_project(e_project_json, token)
 
    #Create Project F
    f_project_json = project_json('Project_F', 'false', c_project_id)
    f_project_id = create_project(f_project_json, token)

    return [f_project_id, e_project_id, d_project_id, c_project_id, b_project_id, a_project_id]
...
def main():
    try:
        ...
        #Granting user A role for projectes C and F
        ...
        #Granting user B role for projectes D and E
        ...
        if (b_project_id not in subtree_as_list or c_project_id not in subtree_as_list or f_project_id not in subtree_as_list or len(subtree_as_list) != 3):
